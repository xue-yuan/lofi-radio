...
const Sidebar: Component = () => {
    ...
    return (
        <>
            <button
                class={`fixed left-0 top-1/2 -translate-y-1/2 z-50 btn btn-sm btn-ghost bg-black/40 backdrop-blur-md border-r border-y border-white/10 rounded-r-lg rounded-l-none text-white/50 hover:text-white hover:pl-4 transition-all duration-300 ${isOpen() ? 'translate-x-[320px]' : 'translate-x-0'}`}
            ...
            <div
                class={`fixed left-0 top-0 bottom-0 w-[320px] bg-black/80 backdrop-blur-xl border-r border-white/10 z-40 transition-transform duration-300 transform ${isOpen() ? 'translate-x-0' : '-translate-x-full'}`}
            >
